...
export function GalleryItem({ item, onOpen }) {
  ...
  const artStyle = {
    ...
    position: 'relative',
    overflow: 'hidden',
    background: item.img ? 'rgba(0,0,0,0.06)' : item.grad,
  ...
  return (
    <div className="gallery__item" onClick={() => onOpen(item)}>
      <div className="gallery__item-inner">
        <div className="gallery__art" style={artStyle}>
          <canvas
            ref={cvRef}
  ...
  return (
    ...
    >
      <div className="lightbox__inner">
        ...
        <div
          style={{
            background: item?.img ? `url(${item.img}) center/cover no-repeat` : item?.grad || '#111',
            maxWidth: '90vw',
            maxHeight: '82vh',
          }}
        >
        ...
        {item && (
          <div className="lightbox__details">
            ...
            <div className="lightbox__specs">
              ...
              <span>Técnica: {item.technique}</span>
            </div>
            <a
              className="lightbox__price-link"
              href={item.priceHref}
              target="_blank"
              rel="noopener noreferrer"
            >
              Preço sob consulta — Enviar WhatsApp
            </a>
          </div>
        )}
      </div>
